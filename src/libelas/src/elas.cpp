...
#endif
  Descriptor desc1(I1,width,height,bpl,param.subsampling_step);
  Descriptor desc2(I2,width,height,bpl,param.subsampling_step);
...
void Elas::addCornerSupportPoints(vector<support_pt> &p_support) {
  ...
  // find closest d
  for (int32_t i=0; i<(int)p_border.size(); i++) {
    int32_t best_dist = 10000000;
    for (int32_t j=0; j<(int)p_support.size(); j++) {
...
vector<Elas::support_pt> Elas::computeSupportMatches (uint8_t* I1_desc,uint8_t* I2_desc) {
  ...
  D_candidate_stepsize += D_candidate_stepsize % param.subsampling_step;
  
  int32_t u_start = 1;
  //int32_t u_start = param.disp_max/D_candidate_stepsize;
  ...
  for (int32_t u_can=u_start; u_can<D_can_width; u_can++) {
...
inline void Elas::findMatch(int32_t &u,int32_t &v,float &plane_a,float &plane_b,float &plane_c,
  ...
  uint32_t d_addr = getAddressOffsetImage(u/param.subsampling_step,v/param.subsampling_step,width/param.subsampling_step);
...
void Elas::computeDisparity(vector<support_pt> p_support,vector<triangle> tri,int32_t* disparity_grid,int32_t *grid_dims,
  ...
  for (int32_t i=0; i<(width/param.subsampling_step)*(height/param.subsampling_step); i++)
    *(D+i) = -1;
...
void Elas::leftRightConsistencyCheck(float* D1,float* D2) {
  ...
  int32_t D_width  = width/param.subsampling_step;
  int32_t D_height = height/param.subsampling_step;
  ...
  for (int32_t u=0; u<D_width; u++) {
    for (int32_t v=0; v<D_height; v++) {
      ...
      d1       = *(D1_copy+addr);
      d2       = *(D2_copy+addr);
      u_warp_1 = (float)u-d1/param.subsampling_step;
      u_warp_2 = (float)u+d2/param.subsampling_step;
...
void Elas::removeSmallSegments (float* D) {
  ...
  // orig code for subsamp: sqrt((float)param.speckle_size)*2
  int32_t D_speckle_size = pow(param.speckle_size, 1.0/param.subsampling_step); // KS TODO: Check results!!!
...
void Elas::gapInterpolation(float* D) {
  ...
  int32_t D_width          = width/param.subsampling_step;
  int32_t D_height         = height/param.subsampling_step;
  int32_t D_ipol_gap_width;
  if (param.subsampling_step > 1)
    D_ipol_gap_width = param.ipol_gap_width/param.subsampling_step+1;
  else D_ipol_gap_width = param.ipol_gap_width;
...
void Elas::adaptiveMean (float* D) {
  ...
  // zero input disparity maps to -1 (this makes the bilateral
  ...
  if (param.subsampling_step > 1) { // KS TODO: Check results
