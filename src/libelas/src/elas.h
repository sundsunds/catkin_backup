...
public:
  ...
  struct parameters {
    ...
    int     subsampling_step;       // saves time by only computing disparities for each 2nd pixel
                                    // note: for this option D1 and D2 must be passed with size
                                    //       width/subsampling_step x height/subsampling_step (rounded towards zero)
  ...
  //               otherwise width/subsampling_step x height/subsampling_step (rounded towards zero)
