...
Descriptor::Descriptor(uint8_t* I,int32_t width,int32_t height,int32_t bpl,int subsampling_step) {
  ...
  createDescriptor(I_du,I_dv,width,height,bpl,subsampling_step);
  _mm_free(I_du);
  _mm_free(I_dv);
}
...
void Descriptor::createDescriptor (uint8_t* I_du,uint8_t* I_dv,int32_t width,int32_t height,int32_t bpl,int subsampling_step) {

  uint8_t *I_desc_curr;  
  uint32_t addr_v0,addr_v1,addr_v2,addr_v3,addr_v4;
  
  // create filter strip
  for (int32_t v=2+subsampling_step; v<height-3; v+=subsampling_step) {
