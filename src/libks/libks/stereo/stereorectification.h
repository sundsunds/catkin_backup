#ifndef KS_STEREORECTIFICATION_H
#define KS_STEREORECTIFICATION_H
...
namespace ks {
	...
	public:
		...
		template <typename T>
		void rectifyStereoPair(const std::pair<cv::Mat_<T>, cv::Mat_<T> >& input, std::pair<cv::Mat_<T>, cv::Mat_<T> >* output, bool replicate = true) const {
			rectifyLeftImage(input.first, &(output->first), replicate);
			rectifyRightImage(input.second, &(output->second), replicate);
		}
		
		// Rectifies the left image
		template <typename T>
		void rectifyLeftImage(const cv::Mat_<T>& input, cv::Mat_<T>* output, bool replicate = true) const {
			cv::remap(input, *output, imageRectMap[0][0], imageRectMap[0][1], interpolation,
				replicate ? cv::BORDER_REPLICATE : cv::BORDER_CONSTANT);
		}
		
		// Rectifies the right image
		template <typename T>
		void rectifyRightImage(const cv::Mat_<T>& input, cv::Mat_<T>* output, bool replicate = true) const {
