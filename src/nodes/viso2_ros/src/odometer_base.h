...
public:
  ...
  OdometerBase()
  {
    ...
    pub_path_vis_    = local_nh.advertise<visualization_msgs::Marker>("/camera_path_vis",1);
    init_path_vis();
...
protected:
  ...
  void integrateAndPublish(const tf::Transform& delta_transform, const ros::Time& timestamp)
  {
    ...
    // Optional: Visualize camera path
    if (true){//(pub_path_vis_.getNumSubscribers() > 0) {
        geometry_msgs::Point p = odometry_msg.pose.pose.position;
        path_.points.push_back(p);

        path_.header.stamp = odometry_msg.header.stamp ;
        pub_path_vis_.publish(path_);
    }
  ...
  // raj world      world → odom → base_link → camera

  void init_path_vis() {
      path_.header.frame_id = "/odom";
      path_.id = 0;
      path_.type = visualization_msgs::Marker::SPHERE_LIST;
