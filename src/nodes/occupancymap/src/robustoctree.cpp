...
void RobustOcTree::insertPointCloud(const octomap::Pointcloud& scan, const octomap::point3d& sensor_origin,
    ...
    //std::cout  <<  "raj  function insertPointCloud point_cloud_size " << std::endl;
...
void RobustOcTree::calcVisibilities(const octomap::point3d& sensor_origin) {
    // Find the origin key
    //std::cout  <<  "raj  function calcVisibilities " << std::endl;
    OcTreeKey originKey;
...
void RobustOcTree::insertPointCloud(const Pointcloud& pc, const point3d& sensor_origin, const pose6d& frame_origin,
    ...
    point3d forwardVec = frame_origin.transform(sensor_origin + point3d(0, 0, 1.0)) - transformed_sensor_origin;
...
void RobustOcTree::computeUpdate(const Pointcloud& scan, const octomap::point3d& origin, const octomap::point3d& forwardVec, double maxrange) {
    ...
    //std::cout  <<  "raj  function computeUpdate " << std::endl;
...
bool RobustOcTree::writeBinary(const std::string& filename){
...
bool RobustOcTree::writeBinary(std::ostream &s) {
    this->prune();
    this->writeBinaryConst(s);
}

bool RobustOcTree::writeBinaryConst(std::ostream &s) const{
...
std::ostream& RobustOcTree::writeBinaryData(std::ostream &s) const {
...
std::ostream& RobustOcTree::writeBinaryNode(std::ostream &s, const RobustOcTreeNode* node) const{
    ...
    for (unsigned int i=0; i<4; i++) {
        if (node->childExists(i)) {
            const RobustOcTreeNode* child = node->getChild(i);
    ...
    for (unsigned int i=0; i<4; i++) {
        if (node->childExists(i+4)) {
            const RobustOcTreeNode* child = node->getChild(i+4);
...
const std::string RobustOcTree::binaryFileHeader = "# Octomap OcTree binary file";
