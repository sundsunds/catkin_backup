...
namespace occupancymap {
	...
	OccupancyMapNodelet::~OccupancyMapNodelet() {
		...
		if(ocTree != NULL && parameters->mapSavePath != "") {
            ...
            ocTree->writeBinary(parameters->mapSavePath + std::string(".bt"));
            fstream strm((parameters->mapSavePath + std::string(".ot")).c_str(),
	...
	void OccupancyMapNodelet::onInit() {
        ...
        //cout << "raj occmap: args calib q" << calib.Q  << endl;

		ocTree.reset(new OcTreeType(parameters));
       visualization.reset(new Visualization(getNodeHandle(), parameters));
       //
		stereoSubscriber.reset(new message_filters::Subscriber<libks_msgs::SharedImageSet>(getNodeHandle(), "/densestereo/disparity_map", 1));
        //raj comment poseSubscriber.reset(new message_filters::Subscriber<geometry_msgs::PoseStamped>(getNodeHandle(), "/imufuse/pose_camera", 1));
		...
		try {
			...
			BOOST_FOREACH(rosbag::MessageInstance const m, view) {
				...
				if(m.getDataType() == "geometry_msgs/PoseStamped") {
					...
					imgs = m.instantiate<libks_msgs::ImageSet>();
				}
	...
	void OccupancyMapNodelet::stereoSynchronizerCallback(const libks_msgs::SharedImageSetConstPtr& multiCam,
    ...
    void OccupancyMapNodelet::stereoSynchronizerCallback_2(cv::Mat_<float> dispMap,
        const geometry_msgs::PoseStamped pose ) {

        //if(multiCam->imagePtrs.size() == 0 || multiCam->imagePtrs[0] == 0)
        if (&dispMap == NULL)
         {
            cout << "Loading octree: No disp image in this message " << endl;
            return; // No images in this message
         }
        static ks::Timer timer("Occupancy Map");
        timer.start();
        ...
        // Convert the point cloud
        //Mat_<float> dispMap = multiCam; //((Mat*)multiCam->imagePtrs[0])->clone();
        disparityMapToScan(dispMap);
        ...
        //std::cout << "raj stereoSynchronizerCallback_2   currentScan size = " << currentScan.size()  <<std::endl;
        //std::cout << "    octomapPose size = " <<  octomapPose.x() << " , "<< octomapPose.y() << " , " << octomapPose.z() <<"  yaw=" << octomapPose.yaw()<<std::endl;
        ocTree->insertPointCloud(currentScan, octomap::point3d(0, 0, 0), octomapPose, parameters->maxPointDist);

        timer.stop();

        // Visualize
        visualization->visualizeAll(dispMap, pointsMap, *ocTree, position, orientation, pose.header.stamp);
    ...
    void OccupancyMapNodelet::set_is_elas(bool is_elas_stereo)
    {
       is_elas = is_elas_stereo;
    }
	
	void OccupancyMapNodelet::disparityMapToScan(const Mat_<float>& dispMap) {

        //cout << "raj fucntion disparityMapToScan()" << endl;
        ...
        for(int y = 0; y < pointsMap.rows; y += parameters->mappingStep) {
            for(int x = 0; x < pointsMap.cols; x += parameters->mappingStep)
            {
                if (dispMap(y,x) > 0)
                {
                   if(finite(pointsMap(y,x).z))
                       currentScan.push_back(pointsMap(y,x).x, pointsMap(y,x).y, pointsMap(y,x).z);
                }
            }
        }
